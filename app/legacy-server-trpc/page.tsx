...
const DashboardContent: React.FC<{ data: ServerData; responseTime: number }> = ({ data, responseTime }) => {
                        ...
                        <div className="flex-1">
                            ...
                            <div className="grid md:grid-cols-2 gap-4 text-blue-700 text-sm">
                                <div>
                                    ...
                                    <p>• Server-side prefetching with hydration</p>
                                </div>
                                <div>
                                    <p>• Responsive design for all devices</p>
                                    <p>• Visual progress indicators</p>
                                    <p>• Server status with smart alerts</p>
                                    <p>• Optimized loading with React Query</p>
                                </div>
                            </div>
                        </div>
                    </div>
...
const Page = async (): Promise<JSX.Element> => {
    try {
        ...
        // Prefetch the data using the query client
        await queryClient.prefetchQuery({
            queryKey: ['hello', { text: "Zaid Server" }],
            queryFn: () => caller.hello({ text: "Zaid Server" }),
        })
        ...
        return (
            <HydrationBoundary state={dehydrate(queryClient)}>
                <DashboardContent data={data} responseTime={responseTime} />
            </HydrationBoundary>
        )
    } catch (error) {
